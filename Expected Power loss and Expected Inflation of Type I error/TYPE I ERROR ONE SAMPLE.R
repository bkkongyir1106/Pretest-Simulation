...
setwd("~/Library/Mobile Documents/com~apple~CloudDocs/Research.05.10.2024/Type I error")
...
{## Set up
  N <- 1e1; threshold <- 0.05
  dist_sum <- c("Standard Normal", "Uniform", "t", "Contaminated", "Exponential", "Laplace", 
                "Chi-Square", "Gamma", "Weibull", "LogNormal", "Pareto")
  #dist_sum = "Exponential"
    ...
    foreach(alpha = sig_level) %dopar%
    {
      ...
      while (TotalSim.passed.SW.test < N) {
        ...
        if(shapiro.test(x)$p.value > alpha){
          ...
          pval[TotalSim.passed.SW.test] <- t.test(x)$p.value
        }
      }
...
error_t.test_par <- prob.non.sig.SW.test_par <- array(powervec,dim = c(length(nvec), length(dist_sum), 
...
for (t in seq_along(nvec)) {
  for (j in seq_along(dist_sum)) {
    for (i in seq_along(sig_level)) {
      error_t.test_par[t, j, i] <- (sim_out[[t]][[j]][[i]]$error_t_test)
      prob.non.sig.SW.test_par[t, j, i] <- (sim_out[[t]][[j]][[i]]$prob.non.sig.SW.test)
    }
  }
}
inflation_error <- error_t.test_par - threshold
error_t.test_par
inflation_error
prob.non.sig.SW.test_par

save.image(paste0("OneSample_Expected_error_par_05_10_2024",".RData"))
#Write the data frame to an Excel file
#write.xlsx(OneSample_inflation_error,  rowNames = T)
#save(prob.non.sig.SW.test_par, error_t.test_par, inflation_error)
