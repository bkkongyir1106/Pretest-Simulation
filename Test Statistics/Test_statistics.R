...
setwd("~/Desktop/OSU/Research/Pretest-Simulation/Test Statistics")
...
message("Running one-sample t-test (unconditional)")
for (i in seq_along(distribution)) {
  ...
  stat_c <- numeric(N)
  count <- 0
  pb2 <- txtProgressBar(min = 0, max = N, style = 3)
  while (count < N) {
    x <- generate_data(n, dist)
...
for (i in seq_along(distribution)) {
  dist <- distribution[i]
  message("Running two-sample t-test (unconditional) for ", dist)
  # Unconditional
  stat <- numeric(N)
  pb3 <- txtProgressBar(min = 0, max = N, style = 3)
  for (j in 1:N) {
    x <- generate_data(n, dist)
  ...
  # Conditional
  message("Running two-sample t-test (conditional) for ", dist)
  stat_c <- numeric(N)
  count <- 0
  pb4 <- txtProgressBar(min = 0, max = N, style = 3)
  while (count < N) {
    x <- generate_data(n, dist)
    y <- generate_data(n, dist)
