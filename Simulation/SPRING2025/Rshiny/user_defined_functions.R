...
OneSample_test_statistic <- function(x, effect_size) {
  return(((mean(x) - effect_size)  * sqrt(length(x))) / sd(x))
}
# calculate two-sample  test statistic 
TwoSample_test_statistic <- function(x, y, effect_size) {
  return((mean(x) - mean(y) - effect_size)/sqrt(var(x)/length(x) + var(y)/length(y)))
}
...
OneSample.test <- function(x, effect_size, test,  alpha, B = NULL){
  ...
  if(test == "perm"){
    observe_stat <- OneSample_test_statistic(x, effect_size = 0)
  ...
  if(test == "2stage.split"){
    ...
    if(shapiro.test(first_half.x)$p.value > alpha){
      pval <- t.test(second_half.x, mu =  effect_size)$p.value
    } else {
      pval <- wilcox.test(second_half.x, mu = effect_size)$p.value
    }
  }
  return(pval)
}
...
TwoSample.test <- function(x, y, effect_size, test, alpha,  B = NULL){
  ...
  if(test == "perm"){
    observe_stat <- TwoSample_test_statistic(x, y, effect_size = 0)
    data <- c(x, y)
    permuted_stat <- numeric(B)
    for (j in 1:B) {
  ...
  if(test == "2stage.split"){
    ...
    second_half.y <- y[(split_point + 1):length(x)] 
    
    if(shapiro.test(first_half.x)$p.value > alpha & shapiro.test(first_half.y)$p.value > alpha){
      pval <- t.test(second_half.x, second_half.y, mu = effect_size)$p.value
    } else {
      pval <- wilcox.test(second_half.x, second_half.y, mu =  effect_size)$p.value
    }
  }
