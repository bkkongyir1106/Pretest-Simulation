...
system.time({
  sim_out <- foreach(i = 1:ntasks,
                     ...
                     .options.snow = opts) %dopar% {
                       ...
                       pval.adaptive_perm.t.test <- c()
                       pval.adaptive.perm.test <- c()
                       count_adaptive.t.test <- 0
                       count_adaptive.boot.test <- 0
                       count_adaptive_split.t.test <- 0
                       count_adaptive_split.boot.test <- 0
                       count_adaptive_perm.t.test <- 0
                       count_adaptive.perm.test <- 0
                       for (j in 1:Nsim) {
                         x <- generate_data(n, dist)
                         y <- generate_data(n, dist)
                         # Perform tests
                         pval.t_test[j] <- TwoSample.test(x, y, test = "t", alpha = alpha, effect_size = effect_size)
                         pval.boot.test[j] <- two_sample_bootstrap_p_value(x, y, effect_size = effect_size, n_bootstrap = n_boot)
                         
                         # Adaptive bootstrap test
                         if (shapiro.test(x)$p.value > alpha) {
                           count_adaptive.t.test <- count_adaptive.t.test + 1
                           pval.adaptive.t.test[count_adaptive.t.test] <- TwoSample.test(x, y, test = "t", alpha = alpha, effect_size = effect_size)
                         } else {
                           count_adaptive.boot.test <- count_adaptive.boot.test + 1
                           pval.adaptive.boot.test[count_adaptive.boot.test] <- two_sample_bootstrap_p_value(x, y, effect_size = effect_size, n_bootstrap = n_boot)
                         }
                         
                         # Adaptive permutation test
                         if (shapiro.test(x)$p.value > alpha) {
                           count_adaptive_perm.t.test <- count_adaptive_perm.t.test + 1
                           pval.adaptive_perm.t.test[count_adaptive_perm.t.test] <- TwoSample.test(x, y, test = "t", alpha = alpha, effect_size = effect_size)
                         } else {
                           count_adaptive.perm.test <- count_adaptive.perm.test + 1
                           pval.adaptive.perm.test[count_adaptive.perm.test] <- TwoSample.test(x, y, test = "perm", alpha = alpha, effect_size = effect_size)
                         }
                         ...
                         second_half.y <- y[(split_point + 1):n]
                         
                         if (shapiro.test(first_half.x)$p.value > alpha) {
                           count_adaptive_split.t.test <- count_adaptive_split.t.test + 1
                           pval.adaptive_split.t.test[count_adaptive_split.t.test] <- TwoSample.test(second_half.x, second_half.y, test = "t", alpha = alpha, effect_size = effect_size)
                         } else {
                           count_adaptive_split.boot.test <- count_adaptive_split.boot.test + 1
                           pval.adaptive_split.boot.test[count_adaptive_split.boot.test] <- two_sample_bootstrap_p_value(second_half.x, second_half.y, effect_size = effect_size, n_bootstrap = n_boot)
                         }
                       }
                       ...
                       ((count_adaptive.boot.test)/Nsim) * mean(pval.adaptive.boot.test < alpha),
                         ...
                         error.adaptive_perm.test = ((count_adaptive_perm.t.test)/Nsim) * mean(pval.adaptive_perm.t.test < alpha) +
                           ((count_adaptive.perm.test)/Nsim) * mean(pval.adaptive.perm.test < alpha)
                       )
                       return(Results)
                     }
})
...
TypeIerror.adaptive_perm.test <- array(powervec, dim = c(length(sample_size), length(distribution)), dimnames = list(sample_size, distribution))
...
for (i in seq_along(sample_size)) {
  for (j in seq_along(distribution)) {
    ...
    TypeIerror.adaptive_perm.test[i, j] <- sim_out[[index]]$error.adaptive_perm.test
  }
}
...
print(TypeIerror.adaptive_perm.test)
