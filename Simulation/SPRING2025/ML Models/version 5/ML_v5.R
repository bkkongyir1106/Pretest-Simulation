...
simulate_predictions <- function(n_iter = 1000, n = 10, dist = "normal",
  ...
  for (i in 1:n_iter) {
    ...
    # Predict with the first model (or change logic for ensemble/majority vote later)
    pred_class <- as.character(predict(trained_models[[1]], newdata = user_features_norm))
    
    if (pred_class == as.character(true_class)) {
      correct_preds <- correct_preds + 1
    }
    
    # Store results
    results[i, ] <- list(i, true_class, pred_class)
  }
  
  accuracy <- correct_preds / n_iter
...
result <- simulate_predictions(
  ...
  n = 10,
  dist = "LogNormal",
  trained_models = list(log_model),
  preProcStandard = norm_result$preProcStandard,
  preProcNorm = norm_result$preProcNorm
)

cat("Overall prediction accuracy across 1000 simulations:", result$accuracy, "\n")
