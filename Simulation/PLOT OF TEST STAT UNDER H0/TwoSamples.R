...
# Define distributions and set parameters
...
# Generate test statistics for each distribution
for (i in seq_along(dist_sum)) {
  ...
  # Calculate type 1 error rates for normal distribution
...
for (i in seq_along(dist_sum)) {
  ...
  p <- ggplot(data, aes(x = stat)) +
    ...
    geom_vline(aes(xintercept = q25, color = "25% Quartile"), linetype = "dashed") +
    geom_vline(aes(xintercept = q95, color = "95% Quartile"), linetype = "dashed") +
    ...
    scale_color_manual(name = NULL,  # Remove legend title
                       ...
                       labels = c("25% Quartile", "95% Quartile", "Empirical Density",  "Theoretical t-Density" )) +
...
dummy_data <- data.frame(x = c(1, 2, 3, 4), 
                         y = c(1, 2, 3, 4), 
                         color = factor(c("Empirical Density", "Theoretical t-Density", "25% Quartile", "95% Quartile")))
...
# Save the workspace
#save.image(paste0("TwoSample_test_stat", ".RData"))
