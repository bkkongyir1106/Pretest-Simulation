    ...
    foreach(d = d.vec) %dopar% {
      ...
      for(i in 1: N) {
        ...
        if (shapiro.test(x)$p.value > alpha) {
          ...
          pval_mann_whitney[i] <- wilcox.test(x)$p.value
          overall_rejections[i] <- wilcox.test(x)$p.value 
        }
      }
      ...
      overall_power <- mean(overall_rejections < alpha, na.rm = TRUE)
...
# Save the results
save(nvec, power_t_test_par, power_mann_whitney_par, overall_power_par, file = "OneSampleOverallPower20240607.RData")