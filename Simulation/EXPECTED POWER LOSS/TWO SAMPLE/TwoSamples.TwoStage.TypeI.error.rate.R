...
system.time({
  sim_out <- foreach(n = nvec, .packages = c("LaplacesDemon", "VGAM"), 
    ...
    foreach(d = sig_level) %dopar% {
      ...
      overall_error <- mean(overall_rejections, na.rm = TRUE)
...
# Save the results
save(nvec, TypeI.error.t.test, TypeI.error.U.test, overall_TypeI.error, file = "TwoSamplesOverallTypeI.error.rate20240607.RData")