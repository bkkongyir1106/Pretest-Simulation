...
system.time({
  sim_out <- foreach(n = nvec,
                     .packages = c("LaplacesDemon", "VGAM"), # install stuborn packages
                     .options.snow=opts) %:% # for progressbar
...
power_perm.test<- power_t.test <- prob.non.sig.SW.test <- array(powervec,dim = c(length(nvec), 
                           length(dist_sum), length(d.vec)), dimnames = list(nvec, dist_sum, d.vec))
for (t in seq_along(nvec)) {
  for (j in seq_along(dist_sum)) {
    for (i in seq_along(d.vec)) {
      power_t.test[t, j, i] <- (sim_out[[t]][[j]][[i]]$power_t_test)
      power_perm.test[t, j, i] <- (sim_out[[t]][[j]][[i]]$power_perm.test)
      prob.non.sig.SW.test[t, j, i] <- (sim_out[[t]][[j]][[i]]$Prob.SW_n.s)
    }
  }
}
# print results
...
# Save the results
save(nvec, power_t.test, power_perm.test, powerloss, prob.non.sig.SW.test, file = "TwoSamplesExpectedPowerLossAND20240607.RData")
