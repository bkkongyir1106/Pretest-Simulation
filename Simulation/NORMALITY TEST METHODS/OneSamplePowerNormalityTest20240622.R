...
{
  N <- 1e4;  alpha <- 0.05
  dist_sum <- c("Normal", "Uniform", "t", "Laplace", "Exponential", 
...
system.time({
  sim_out <- foreach(n = nvec,
    ...
    foreach(test = testvec) %dopar% {   # renamed loop variable from testvec to test
      set.seed(12345)  # Note: this resets the RNG for each (n, dist, test) combination.
      pval <- numeric(N)
      for (i in 1:N) {
