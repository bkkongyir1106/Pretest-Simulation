...
# alpha.level = seq(from = 0, to = 0.5, by = 0.01)  # Range of significance levels
# 
...
### ROC Curve for Shapiro-Wilk Test
```{r}
load("/Users/benedictkongyir/Desktop/OSU/Research/Pretest-Simulation/Simulation/NORMALITY TEST METHODS/ROC_SW.test.RData")
# Plot the ROC curves
plot(FPR, TPR_exp, type = "l", col = "blue", lwd = 3, ylim = c(0, 1), xlab = "False Positive Rate (FPR)", ylab = "True Positive Rate (TPR)", main = "ROC Curve for Shapiro-Wilk Test", lty = 1, pch = 1)
lines(FPR, TPR_chisq, col = "red", lwd = 3, lty = 2, pch = 2)
lines(FPR, TPR_lognorm, col = "purple", lwd = 3, lty = 3, pch = 3)
#lines(FPR, TPR_unif, col = "orange", lwd = 2)
#lines(FPR, TPR_laplace, col = "brown", lwd = 2)
abline(a = 0, b = 1, lty = 2, col = "green")  # Diagonal reference line

# Add a legend
legend("bottomright", title = "Distribution", legend = c("Exponential", "Chi-Squared", "Lognormal"), col = c("blue", "red", "purple"), lwd = 2, lty = 1:3)
```
...
# N = 1e4       # Number of repetitions for each alpha level to average TPR and FPR
# n = 10        # Sample size for each test
# alpha.level = seq(from = 0, to = 0.1, by = 0.01)  # Range of significance levels
# 
...
### ROC Curves for Different Normality Tests
```{r}
...
tests = c("SW", "KS", "DAP", "SF", "JB")
line_types <- c(1, 2, 3, 4, 5)  # Different line types for each test
...
for (i in seq_along(tests)) {
  lines(FPR_list[[tests[i]]], TPR_list[[tests[i]]], col = colors[i], lwd = 3, lty = line_types[i])
}
...
### ROC Curves for t-test vs two-stage method
```{r}
# load data 
load("/Users/benedictkongyir/Desktop/OSU/Research/Pretest-Simulation/Simulation/Fall 2024/ROC Curves/t.test_vs_two_stage_ROC.RData")
...
lines(FPR_w_t, TPR_w_t, type = "l", col = "red", lwd = 2, lty = 2)
...
# library(moments)
# # Parameters
# N <- 1e4             # Increased number of repetitions for smoother ROC curve
# n <- 10              # Sample size for each test
# d <- 0.85            # Effect size
# sw.alpha <- 0.05     # Significance level for the skewness test (based on a theoretical threshold)
# 
# # Range of significance levels for ROC curve
# alpha.level <- seq(0.001, 0.1, 0.01)
# 
...
#     pval_powr_t[j] <- t.test(x - 1, y - 1 + d)$p.value
# 
#     # Test for normality using skewness test
#     if (abs(skewness_test(x)) < 0.5 && abs(skewness_test(y)) < 0.5) {
#       # Use t-test if both samples pass skewness normality test
#       pval_error_w_t[j] <- t.test(x - 1, y - 1)$p.value
#       pval_powr_w_t[j] <- t.test(x - 1, y - 1 + d)$p.value
#     } else {
#       # Use Wilcoxon test if normality assumption is violated
...
#   TPR_w_t[i] <- mean(pval_powr_w_t < alpha)
# }
# save(FPR_t, TPR_t, FPR_w_t, TPR_w_t, file = "t.test_vs_two_stage_Skewness_ROC.RData")
```

### ROC Curves for t-test vs two-stage method
```{r}
# load data 
load("/Users/benedictkongyir/Desktop/OSU/Research/Pretest-Simulation/Simulation/Fall 2024/ROC Curves/t.test_vs_two_stage_Skewness_ROC.RData")
# Plot the ROC curves with smoothing
plot(FPR_t, TPR_t, type = "l", col = "blue", lwd = 2, ylim = c(0, 1),
     xlab = "False Positive Rate (FPR)", ylab = "True Positive Rate (TPR)",
     main = "ROC Curve for t vs two-stage method for Exponential Data")
lines(FPR_w_t, TPR_w_t, type = "l", col = "red", lwd = 2, lty = 5)
abline(a = 0, b = 1, lty = 2, col = "grey")  # Diagonal reference line for random guessing
