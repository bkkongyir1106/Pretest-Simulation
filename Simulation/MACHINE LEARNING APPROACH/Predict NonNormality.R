...
generate_data <- function(n_samples, sample_size, label, dist = "normal") {
  data <- do.call(rbind, lapply(1:n_samples, function(x) {
    samples <- if (dist == "normal") {
      rnorm(sample_size)
    } else if (dist == "normal_25_12") {
      rnorm(sample_size, mean = 25, sd = sqrt(12))
    } else {
      generate_nonnormal_samples(sample_size, dist)
    }
...
set.seed(123)
normal_data <- rbind(
  generate_data(100, 8, "Normal", "normal"),
  generate_data(100, 8, "Normal", "normal_25_12")
)
