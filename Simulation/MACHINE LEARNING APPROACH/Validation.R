...
# Generate data
set.seed(123)
normal_data <- generate_data(100, 10, "normal", "Normal")
lognorma <- generate_data(50, 10, "LogNormal", "Non_Normal")
chisq_data <- generate_data(500, 10, "Chi-Square", "Non_Normal")
...
# Step 7: Model Validation using New Distribution
# # Generate validation data from a new distribution (e.g., Gamma distribution)
# generate_validation_data <- function(n_samples, n_per_sample, label) {
#   data <- do.call(rbind, lapply(1:n_samples, function(x) {
#     samples <- rgamma(n_per_sample, shape = 2, scale = 2)
#     features <- calculate_features(samples)
#     features$Label <- label
#     return(features)
#   }))
...
set.seed(123)
validation_normal <- generate_data(100, 10, "normal", "Normal")
validation_gamma <- generate_data(500, 10, "Laplace", "Non_Normal")

# Combine the validation data
validation_data <- rbind(validation_normal, validation_gamma)
