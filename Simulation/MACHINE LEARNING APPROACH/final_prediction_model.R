...
# Generate data
set.seed(12345)
normal_data <- generate_data(10, 900, "normal", "Normal")
...
spike <- generate_data(10, 100, "spike", "Non_Normal")
cauchy <- generate_data(10, 100, "cauchy", "Non_Normal")
Uniform <- generate_data(10, 100, "Uniform", "Non_Normal")
#combine data
non_normal_data <- rbind(lognormal,chisq_data, exp_data, Weibull, Laplace, Gamma, spike, cauchy, Uniform)
...
train_dataset <- Normalized_train_data[sample(nrow(Normalized_train_data)), ]
test_dataset <- Normalized_test_data[sample(nrow(Normalized_test_data)), ]

# Step 4: Build Machine Learning Models
# Logistic Regression Model
log_model <- train(Label ~ ., data = train_dataset, method = "glm", family = "binomial")
log_pred <- predict(log_model, newdata = test_dataset)
log_pred <- factor(log_pred, levels = levels(test_dataset$Label))
log_conf_matrix <- confusionMatrix(log_pred, test_dataset$Label)
print(log_conf_matrix)

# Random Forest Model
rf_model <- train(Label ~ ., data = train_dataset, method = "rf")
rf_pred <- predict(rf_model, newdata = test_dataset)
rf_pred <- factor(rf_pred, levels = levels(test_dataset$Label))
rf_conf_matrix <- confusionMatrix(rf_pred, test_dataset$Label)
print(rf_conf_matrix)

# ANN Model
ann_model <- train(Label ~ ., data = train_dataset, method = "nnet", trace = FALSE)
ann_pred <- predict(ann_model, newdata = test_dataset)
ann_pred <- factor(ann_pred, levels = levels(test_dataset$Label))
ann_conf_matrix <- confusionMatrix(ann_pred, test_dataset$Label)
print(ann_conf_matrix)

# Gradient Boosting Trees (GBM)
gbm_model <- train(Label ~ ., data = train_dataset, method = "gbm", verbose = FALSE)
gbm_pred <- predict(gbm_model, newdata = test_dataset)
gbm_pred <- factor(gbm_pred, levels = levels(test_dataset$Label))
gbm_conf_matrix <- confusionMatrix(gbm_pred, test_dataset$Label)
print(gbm_conf_matrix)

# Support Vector Machines (SVM)
svm_model <- train(Label ~ ., data = train_dataset, method = "svmRadial", trControl = trainControl(classProbs = TRUE))
svm_pred <- predict(svm_model, newdata = test_dataset)
svm_pred <- factor(svm_pred, levels = levels(test_dataset$Label))
svm_conf_matrix <- confusionMatrix(svm_pred, test_dataset$Label)
print(svm_conf_matrix)

# K-Nearest Neighbors (KNN)
knn_model <- train(Label ~ ., data = train_dataset, method = "knn")
knn_pred <- predict(knn_model, newdata = test_dataset)
knn_pred <- factor(knn_pred, levels = levels(test_dataset$Label))
knn_conf_matrix <- confusionMatrix(knn_pred, test_dataset$Label)
print(knn_conf_matrix)

# Create a function to plot ROC curves
plot_roc_curve <- function(models, test_dataset) {
  roc_curves <- list()
  for (model_name in names(models)) {
    model <- models[[model_name]]
    pred_prob <- predict(model, newdata = test_dataset, type = "prob")[,2]
      roc_curve <- roc(test_dataset$Label, pred_prob, levels = rev(levels(test_dataset$Label)))
...
# Plot the ROC curves
plot_roc_curve(models, test_dataset)
