...
source("~/Desktop/OSU/Research/Pretest-Simulation/User_framework/one sample results/one_sample_test_functions.R")

setwd("~/Desktop/OSU/Research/Pretest-Simulation/User_framework/one sample results")
  ...
  for (method in ds_test_methods) {
    ...
    for (i in seq_along(sample_sizes)) {
      ...
      for (sim in seq_len(Nsim)) {
        # Build params for H0 and H1 
        paras_H0 <- get_parameters(n, par = NULL, effect_size = 0.0)
...
# calculate AUCs
auc_power <- sapply(power_results, function(y) compute_area(sizes, y))
auc_type1 <- sapply(TypeI_error_results, function(y) compute_area(sizes, y))

# create a table of AUCs
...
pdf("one_sample_test_power_and_typeI.pdf", width = 12, height = 6)
plot_pch = c(19,17,15)
plot_col <- c("red","blue","green")
op <- par(no.readonly = TRUE); on.exit(try(par(op), silent = TRUE), add = TRUE)
par(mfrow = c(1, 2), mar = c(4.2, 4.5, 3, 1))

# Power panel (left)
plot(sizes, power_results$Parametric, type="b", pch=plot_pch[1], col=plot_col[1], lwd=2, ylim=c(0,1), xlab="Sample Size", ylab="Power", main="Power vs Sample Size")
lines(sizes, power_results$Nonparametric, type="b", pch=plot_pch[2], col=plot_col[2],  lwd=2)
lines(sizes, power_results$Adaptive,      type="b", pch=plot_pch[3], col=plot_col[3], lwd=2)
abline(h = 0.8, col = "gray50", lty = 2, lwd = 1.5)
legend("bottomright", legend = paste0(c("Parametric","Nonparametric","Adaptive")," (AUC=", formatC(auc_power[c("Parametric","Nonparametric","Adaptive")],format="f", digits=3), ")"),col = plot_col, pch = plot_pch, lty = 1, lwd = 2, bty = "b")

# Type I error panel (right)
plot(sizes, TypeI_error_results$Parametric, type="b", pch=plot_pch[1], col=plot_col[1], lwd=2, ylim=c(0, max(0.12, TypeI_error_results$Parametric, TypeI_error_results$Nonparametric, TypeI_error_results$Adaptive, na.rm=TRUE)), xlab="Sample Size", ylab="Type I Error", main="Type I Error vs Sample Size")
lines(sizes, TypeI_error_results$Nonparametric, type="b", pch=plot_pch[2], col=plot_col[2],  lwd=2)
lines(sizes, TypeI_error_results$Adaptive,      type="b", pch=plot_pch[3], col=plot_col[3], lwd=2)
abline(h = alpha, col = "gray40", lty = 3, lwd = 1.5)
legend("topleft",legend = paste0(c("Parametric","Nonparametric","Adaptive")," (AUC=", formatC(auc_type1[c("Parametric","Nonparametric","Adaptive")], format="f", digits=3), ")"),col = plot_col, pch = plot_pch, lty = 1, lwd = 2, bty = "b")
...
# ------------------------------------------------------------------------------
roc_curve_for_norm_test <- function(FPR, TPR, tests_to_plot = rownames(FPR), alpha = NULL,title = "ROC Curves for Different Normality Tests") {
  ...
  plot(0, 0, type = "n", xlim = c(0, 1), ylim = c(0, 1), xlab = "False Positive Rate (FPR)", ylab = "True Positive Rate (TPR)", main = title)
  ...
  legend("bottomright", legend = tests_to_plot, col = colors, pch = plot_chars, lwd = 2, title = "Normality Tests")
}

# Plot ROC using selected tests
pdf("one_sample_test_norm_roc_curve.pdf", width=12, height=8)
...
generate_pval<- function(N, n, effect_size, test = "SW", dist = "Normal", ...) {
  # Initialize storage 
  pval_ds_test1_H0 <- pval_ds_test2_H0 <- numeric(N)
...
perform_analysis <- function(N, n, distributions, effect_size, test, alpha_pretest, test_alpha) {
  ...
  for(dist_idx in seq_along(distributions)) {
    ...
    power_ds_test[[dist]] <- list(
      power_ds_test1 = mean(ds_test_results[[dist]]$pval_t.test_H1 < test_alpha),
      power_ds_test2 = mean(ds_test_results[[dist]]$pval_wilcox.test_H1 < test_alpha),
    ...
    for(j in seq_along(alpha_pretest)) {
      ...
      adaptive_pvals_H0 <- ifelse(use_ds_test1_H0,
      ...
      adaptive_pvals_H1 <- ifelse(use_ds_test1_H1,
...
# run analysis to get power and Type I error
Nsim <- 1e5
...
compute_roc_metrics <- function(error_ds_test, power_ds_test, test_alpha) {
  ...
  # Non-normal case
  power_t_test_nonnormal <- power_ds_test[[ 2 ]]$power_ds_test1
  adaptive_power_nonnormal <- power_ds_test[[ 2 ]]$power_adaptive_test
  adaptive_error_nonnormal <- error_ds_test[[ 2 ]]$error_adaptive_test
  # reverse calculation for EPG
  EPG <- -(adaptive_power_nonnormal - power_t_test_nonnormal) 
  EDE <- (adaptive_error_nonnormal - test_alpha)
  
  # normal case
  power_t_test_normal <- power_ds_test[[ 1 ]]$power_ds_test1
  adaptive_power_normal <- power_ds_test[[ 1 ]]$power_adaptive_test
  adaptive_error_normal <- error_ds_test[[ 1 ]]$error_adaptive_test
  ...
  power_gain <- power_ds_test[[ 2 ]]$power_ds_test2 - power_ds_test[[ 2 ]]$power_ds_test1
  power_loss <- power_ds_test[[ 1 ]]$power_ds_test1 - power_ds_test[[ 1 ]]$power_ds_test2
...
# Generate all plots
pdf("one_sample_test_tradeoff_v2.pdf", width=12, height=8)
...
pdf("one_sample_test_EPL_vs_EPG_v2.pdf", width=12, height=8)
...
generate_roc_tables <- function(N, n, distributions, effect_size, sig_levels) {
  ...
  for (dist in distributions) {
    ...
    for (alpha in sig_levels) {
      ...
      # adaptive test p-value under H0
      ...
      error_t <- mean(res$pval_ds_test1_H0 <= alpha)
      error_w <- mean(res$pval_ds_test2_H0 <= alpha)
      error_adaptive <- mean(adaptive_pvals_H0 <= alpha)
      
      # adaptive test p-value under H1
      use_ds_test1_H1 <- sapply(res$norm_pvals_H1, function(v) all(as.numeric(v) > alpha_pretest))
      adaptive_pvals_H1 <- ifelse(use_ds_test1_H1, res$pval_t.test_H1, res$pval_wilcox.test_H1)
      ...
      roc_data <- rbind(roc_data, 
                        data.frame(Distribution = dist, Method = "ds_test 1", Alpha = alpha, Power = power_t, TypeIError = error_t
                        ),
                        data.frame(Distribution = dist, Method = "ds_test 2", Alpha = alpha,Power = power_w,TypeIError = error_w
                        )
                        ,
                        data.frame(Distribution = dist, Method = "adaptive test",Alpha = alpha, Power = power_adaptive, TypeIError = error_adaptive
                        ))
...
pdf("one_sample_test_power_error.pdf", width=12, height=6)
...
save.image(paste0("one_sample_test_results_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".RData"))
