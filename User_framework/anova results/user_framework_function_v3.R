...
setwd("~/Desktop/OSU/Research/Pretest-Simulation/User_framework/anova results")

# general source
#source("~/Desktop/OSU/Research/Pretest-Simulation/User_framework/sample_test_functions/two_sample_t_test_functions/two_sample_test_fns.R")

source("~/Desktop/OSU/Research/Pretest-Simulation/User_framework/sample_test_functions/anova_test_functions/anova_test_functions.R")
#source("~/Desktop/OSU/Research/Pretest-Simulation/User_framework/sample_test_functions/one_sample_test/one_sample_test_functions.R")
...
alpha       <- 0.05
norm_test   <- "SW"
effect_size <- c(0, 0.2, 0.0, 0.0, 0.5)
ds_test_methods <- c("parametric", "nonparametric", "adaptive")
...
pdf("anova_test_power_and_typeI.pdf", width = 12, height = 6)
...
# Generate all plots
pdf("anova_test_tradeoff.pdf", width=12, height=8)
...
EPL_vs_EPG_ROC_like_curve <- function(metrics){
  ...
  plot(metrics$EPL,
       ...
       ylab = "EPL(Non-normal)", 
       xlab = "EPL(Normal)",
       main = "ROC like curve: Power Loss/Gain")
  ...
  plot(metrics$EDE,
       ...
       ylab = "EIE(Normal)", 
       xlab = "EDE(Non-Normal)",
       main = "ROC like curve: I/D of Type I error")
}

pdf("anova_test_EPL_vs_EPG.pdf", width=12, height=8)
...
generate_roc_tables <- function(N, n, distributions, effect_size, sig_levels) {
  ...
  for (dist in distributions) {
    ...
    for (alpha in sig_levels) {
      ...
      # H1 gate (Power) — MUST be based on H1 data
...
pdf("anova_test_power_error.pdf", width=12, height=8)
...
save.image(paste0("anova_test_results_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".RData"))
