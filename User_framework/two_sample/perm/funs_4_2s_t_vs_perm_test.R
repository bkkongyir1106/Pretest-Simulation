...
two_sample_permutation_test <- function(x, y, B) {
  ...
  perm_stats <- replicate(B, {
    ...
    x_star <- permuted_sample[1:length(x)]
    y_star <- permuted_sample[(length(x)+1):(2*length(x))]
...
# test 2: Two Sample Permutation test
fn_for_ds_test_2 <- function(data, B = 1000) {
  x <- data$value[data$group == "x"]
  y <- data$value[data$group == "y"]
  # observe stat
  observed <- (mean(x) - mean(y))/(sqrt(var(x)/length(x) + sqrt(y)/length(y)))
  combined <- c(x, y)
  # perm stat
  perm_stats <- replicate(B, {
    ...
    (mean(x_star) - mean(y_star))/(sqrt(var(x_star)/length(x_star) + sqrt(y_star)/length(y_star)))
  })
  
  pvalue <- mean(abs(perm_stats) >= abs(observed))
...
# -----------------------------------------------------------------------------
generate_pval<- function(Nsim, n, effect_size, test = "SW", dist = "Normal", ...) {
...
power_vs_error_roc_data <- function(N, n, distributions = c("Non-normal", "Normal"), effect_size, alpha_pretest = 0.05, sig_levels) {
  ...
  for (dist in distributions) {
    ...
    for (alpha in sig_levels) {
      ...
      roc_data <- rbind(
        roc_data,
        data.frame(Distribution = dist, Method = "ds_test 1", Alpha = alpha, Power = power_test1, Expected_typeI_error_inflation = type1_error_test1),
        data.frame(Distribution = dist, Method = "ds_test 2", Alpha = alpha, Power = power_test2, Expected_typeI_error_inflation = type1_error_test2),
        data.frame(Distribution = dist, Method = "adaptive test", Alpha = alpha, Power = power_adaptive,Expected_typeI_error_inflation = type1_error_adaptive)
      )
      counter <- counter + 1 
      setTxtProgressBar(pb, counter)
    }
...
# ------------------------------------------------------------------------------
power_vs_error_ROC_curve <- function(roc_data, file = NULL, width = 8, height = 6.8, alpha_star = 0.05, zoom_xlim = 0.10,
                                     subtitle = "Power vs Type I error: full view (first row) and zoom view (second row)"){
  ...
  plot_panel <- function(dist, xlim, bottom_mar = 2.0) {
    ...
    for (i in seq_along(methods)) {
      ...
      lines(data_sub$Expected_typeI_error_inflation, data_sub$Power, col = colors[i], lwd = 2)
      
      # Mark alpha_star point
      idx <- which.min(abs(data_sub$Alpha - alpha_star))
      if (length(idx) == 1 && is.finite(idx)) {
        points(data_sub$Expected_typeI_error_inflation[idx], data_sub$Power[idx], pch = point_char, col = colors[i])
      }
    }
  ...
  add_legend <- function(dist) {
    ...
    legend_labels <- sapply(seq_along(methods), function(i) {
      ...
      idx <- which.min(abs(data_sub$Alpha - alpha_star))
      x <- data_sub$Expected_typeI_error_inflation[idx]
...
# ----------------------------------------------------------------------------
perform_ds_power_by_effect <- function(fixed_n = 10, effect_sizes = NULL, 
                                       distributions = c("Non-normal", "Normal"), 
                                       N = N, alpha = alpha, gen_data = gen_data,
