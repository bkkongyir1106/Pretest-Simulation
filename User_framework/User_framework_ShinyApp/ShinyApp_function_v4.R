...
setwd("~/Desktop/OSU/Research/Research/user_framework/ShinyApp")
...
perform_ds_func <- function(
  ...
  for (method in ds_test_methods) {
    ...
    for (i in seq_along(sample_sizes)) {
      ...
      cat("Completed n =", n, "| Method =", method, "| results =", ds_test_results[i], "\n")
    }
...
perform_analysis <- function(N, n, distributions, effect_size, test, alpha_pretest, test_alpha) {
  ...
  for(dist in distributions) {
    ...
    ds_test_results[[dist]] <- generate_pval(N, n, effect_size, test = "SW", dist = dist)
...
plot_power_error_tradeoff <- function(alpha_pretest, metrics, dist = distributions,  file_name = NULL) {
  ...
  plot(alpha_pretest, 
       ...
       main = paste("Power Loss for", dist[[1]]))
  ...
  plot(alpha_pretest, 
       ...
       xlab = expression(alpha),
       main = paste("Power Gain for", dist[[2]]))
  ...
  plot(alpha_pretest, 
       ...
       xlab = expression(alpha),
       main = paste("Expected Inflation of Type I error", dist[[1]]))
  ...
  plot(alpha_pretest, 
       ...
       ylab = "Type I Error Inflation", 
       xlab = expression(alpha),
       main = paste("Expected Inflation of Type I error", dist[[2]]))
...
# ROC like curves for expected power loss/ inflation of Type I error
...
run_test_simulation <- function(test_type, effect_size, H1_dist) {
  # Set file paths based on test type
  FilePath <- paste0(
    "~/Desktop/OSU/Research/Research/user_framework/ShinyApp/sample_test_functions/",
    test_type, "_functions/"
  )
  ...
  test_config <- list(
    test_name = switch(test_type,
                       "t_test" = "Two Sample Location Test",
                       "anova" = "ANOVA Test",
  ...
  par(mfrow = c(1,2))
  # Run ROC for normality test
  roc_pval_ds_test <- fn_for_roc_curve_for_norm_test(
    n = 10,
    alpha_pretest = alpha_pretest,
    H1_dist = test_config$H1_dist,
    tests = c("SW", "SF", "JB"), #, "DAP", "AD", "CVM", "SKEW"),
  
    Nsim = Nsim
  )
  
  # Plot ROC
  selected_tests <- c("SW", "SF", "JB") #, "DAP", "CVM")
  roc_curve_for_norm_test(
    FPR = roc_pval_ds_test$FPR,
    TPR = roc_pval_ds_test$TPR,
    tests_to_plot = selected_tests,
    alpha = roc_pval_ds_test$alpha,
    title = paste("ROC for Normality Test (", test_config$test_name, ")")
  )
  ...
  ds_test_results <- perform_ds_func(
    ...
    ds_test_methods = c("parametric", "nonparametric", "adaptive"),
    effect_size = test_config$effect_size
  )
  ...
  auc_df <- data.frame(
    ...
    AUC = c(
      compute_area(sample_sizes, ds_test1_results),
      compute_area(sample_sizes, ds_test2_results),
      compute_area(sample_sizes, ds_adaptive_results)
    )
  )
  
  # Plot results
  plot(sample_sizes, ds_test1_results,
       type = "b", pch = 19, ylim = c(0, 1),
       xlab = "Sample Size", ylab = "Power", col = "red",
       main = test_config$test_name)
  lines(sample_sizes, ds_test2_results, type = "b", pch = 17, col = "blue")
  lines(sample_sizes, ds_adaptive_results, type = "b", pch = 15, col = "green")
  
  legend("bottomright",
         legend = c("Parametric", "Nonparametric", "Adaptive"),
  ...
  return(list(
    results = list(
      sample_sizes = sample_sizes,
      parametric = ds_test1_results,
      nonparametric = ds_test2_results,
      adaptive = ds_adaptive_results,
      auc = auc_df
    ),
    metrics = metrics
  ))
...
# ----------------- Run Tests -----------------
# Save t-test plots
pdf("t_test_plots.pdf", width=12, height=8)
t_test_results <- run_test_simulation(
  test_type = "t_test",
  effect_size = 0.5,
  H1_dist = "Exponential"
)
...
# Save ANOVA plots
pdf("anova_plots.pdf", width=12, height=8)
anova_results <- run_test_simulation(
  test_type = "anova",
...
pdf("regression_plots.pdf", width=12, height=8)
regression_results <- run_test_simulation(
