...
jpeg(file="error_unif_strategy I.jpeg")

myplot<-plot(sample_size,pvalue[,1], type = "l", pch=24,lwd=3,ylim = c(0.00,0.25), 
             ylab = "Type I Error", xlab = "Sample Size")
title(main = "Type I Error Rate for samples from unif dist under strategy I")
...
error.unif_t_II <- as.data.frame(pvalue)

#export data into excel
library("writexl")
write_xlsx(error.unif_t_II, "D:/OSU/Research/Simulations/Type I error rate under strategy I/error.unif_t_I.xlsx")

save.image(paste0("error.unif_t_I",".RData"))