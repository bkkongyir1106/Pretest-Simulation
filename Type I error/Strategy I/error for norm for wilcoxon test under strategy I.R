setwd("D:/OSU/Research/Simulations/Type I error rate under strategy I")
set.seed(1)
alpha<-0.05
...
jpeg(file="error_norm_strategy I_w.test.jpeg")

myplot<-plot(sample_size,pvalue[,1], type = "l", pch=24,lwd=3,ylim = c(0.00,0.25), 
             ylab = "Type I Error", xlab = "Sample Size")
title(main = "Type I Error Rate of wilcoxon test for samples from norm dist under strategy I")
...
error.norm_w_II <- as.data.frame(pvalue)

#export data into excel
library("writexl")
write_xlsx(error.norm_w_II, "D:/OSU/Research/Simulations/Type I error rate under strategy I/error.norm_w_I.xlsx")

save.image(paste0("error.norm_w_I",".RData"))