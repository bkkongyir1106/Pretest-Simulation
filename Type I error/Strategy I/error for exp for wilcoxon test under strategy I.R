...
jpeg(file="error_exp_strategy I_w.jpeg")

myplot<-plot(sample_size,pvalue[,1], type = "l", pch=24,lwd=3,ylim = c(0.00,0.25), 
             ylab = "Type I Error", xlab = "Sample Size")
title(main = "Type I Error Rate for wilcoxon test for samples from exp dist under strategy I")
...
error.exp_w_II <- as.data.frame(pvalue)

#export data into excel
library("writexl")
write_xlsx(error.exp_w_II, "D:/OSU/Research/Simulations/Type I error rate under strategy I/error.exp_w_I.xlsx")

save.image(paste0("error.exp_w_I",".RData"))