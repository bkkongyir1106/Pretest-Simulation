...
system.time({
  sim_out <- foreach(n = nvec,
    ...
    foreach(alpha = sig_level) %dopar%
    {
      ...
      prob.non.sig.SW <-   TotalSim.passed.SW.test/TotalSim
      Results <- list(
        error_t_test = error_t_test,
        prob.non.sig.SW = prob.non.sig.SW
      )
      return(Results)
    }
})
...
for (t in seq_along(nvec)) {
  for (j in seq_along(dist_sum)) {
    for (i in seq_along(sig_level)) {
      ...
      prob.non.sig.SW.test[t, j, i] <- (sim_out[[t]][[j]][[i]]$prob.non.sig.SW)
    }
  }
}
...
#save results
save.image(paste0("ONESAMPLE.TYPEI.ERROR20240611*",".RData"))

save(nvec, TypeI.Error.T.Test, Inflation.TypeI.Eerror, prob.non.sig.SW.test, file = "ONESAMPLE.TYPEI.ERROR20240611.RData")
