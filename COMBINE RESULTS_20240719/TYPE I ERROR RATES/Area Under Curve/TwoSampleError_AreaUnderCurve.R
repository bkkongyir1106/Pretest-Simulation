...
system.time({
  sim_out <- foreach(n = nvec,
    ...
    foreach(dist = dist_sum) %dopar%
    {
      ...
      results <- list(error_t = error_t,error_wilcox = error_wilcox, error_t_wilcox = error_t_wilcox,error_perm = error_perm,
                      time_t = time_t, time_wilcox = time_wilcox, time_t_wilcox = time_t_wilcox, time_perm = time_perm)
    }
