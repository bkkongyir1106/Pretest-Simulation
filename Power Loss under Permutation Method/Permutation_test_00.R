alpha <- 0.05; N= 1e4; P = 1e4;  d = 0.5
n.vec = c(10, 20, 30, 40, 50)
set.seed(2024)

power_t_test_normal <- power_t_test_exp <- power_t_test_t <- power_t_test_logn <-
  power_t_test_chisq <- power_t_test_weibull <- power_t_test_beta <- power_t_test_gamma<- numeric(length(n.vec))

for(k in 1 : length(n.vec)){
  n = n.vec[k]
  print(n)
  powr_ttest_n <-powr_ttest_e <- powr_ttest_t <- powr_ttest_logn <- powr_ttest_gamma <-
    powr_ttest_chisq <- powr_ttest_weibull <- powr_ttest_beta <- numeric(N)
  for(j in 1 : N){
    # normal distn
    n1 <- rnorm(n, mean = 0, sd = 1)
    n2 <- rnorm(n, mean = 0, sd = 1) + d
    if(t.test(n1, n2, var.equal = F)$p.value < alpha){
      powr_ttest_n[j] <- 1
    }
    # exp distn
    e1 <- rexp(n, rate = 1)
    e2 <- rexp(n, rate = 1) + d
    if(t.test(e1, e2, var.equal = F)$p.value < alpha){
      powr_ttest_e[j] <- 1
    }
    # t distn
    t1 <-rt(n, df = 3) 
    t2 <-rt(n, df = 3) + d
    if(t.test((t1)/sqrt(3), (t2)/sqrt(3), var.equal = F)$p.value <= alpha){
      powr_ttest_t[j] <- 1
    }
    # lognormal distn
    ...
    if(t.test(z1, z2, var.equal = F)$p.value <= alpha){
      powr_ttest_logn[j] <- 1
    }
    ...
    if(t.test((g1-30)/sqrt(300), (g2-30)/sqrt(300) ,var.equal = F)$p.value <= alpha){
      powr_ttest_gamma[j] <- 1
    }
    # Chi-squared
    k1 <-rchisq(n, df = 7) 
    k2 <-rchisq(n, df = 7) 
    if(t.test((k1 - 7)/sqrt(14), (k2 - 7)/sqrt(14) + d, var.equal = F)$p.value <= alpha){
  ...
  if(t.test(w1 , w2 + d, var.equal = F)$p.value <= alpha){
  ...
  if(t.test(b1, b2 + d, var.equal = F)$p.value <= alpha){
    powr_ttest_beta[j] <- 1
  }
...
power_perm_test_exp <- power_perm_test_t <- power_perm_test_logn <- power_perm_test_gamma <-
  power_perm_test_chisq <- power_perm_test_weibul <- power_perm_test_beta<- numeric(length(n.vec))
for(k in 1 : length(n.vec)){
  n <- n.vec[k]
  print(n)
  # exp distn
...
# power under t test
power_t<-c(power_t_test_normal, power_t_test_exp, power_t_test_t,  power_t_test_logn, power_t_test_gamma,
           power_t_test_chisq, power_t_test_weibull, power_t_test_beta)
...
power_perm<-c(power_perm_test_exp, power_perm_test_t, power_perm_test_logn,  power_perm_test_gamma, 
              power_perm_test_chisq, power_perm_test_weibul, power_perm_test_beta)
power_table_perm<-array(power_perm, dim = c(5, 7), dimnames = list(n.vec,
             c("exp_perm", "t_perm",  "lognormal_perm", "gamma_perm", "chisq", "weibul", "beta")))
power_table_perm
...
save.image(paste0("perm_result",".RData"))
