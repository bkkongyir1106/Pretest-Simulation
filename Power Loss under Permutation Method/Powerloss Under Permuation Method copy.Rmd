...
n.perm.test_p_value <- e.perm.test_p_value <- t.perm.test_p_value <-
  logn.perm.test_p_value <- gamma.perm.test_p_value  <- numeric(length(n.vec))
pwr_perm_norm <- numeric(length(n.vec))
...
n.permuted_statistics <- e.permuted_statistics <- t.permuted_statistics <- 
  logn.permuted_statistics <- gamma.permuted_statistics<- numeric(N)
for (i in 1:N) {
...
n.perm.test_p_value[k] <- round(mean(abs(n.permuted_statistics) >= abs(n.observed_statistic)), 3) 
e.perm.test_p_value[k] <- round(mean(abs(e.permuted_statistics) >= abs(e.observed_statistic)), 3)
t.perm.test_p_value[k] <- round(mean(abs(t.permuted_statistics) >= abs(t.observed_statistic)), 3)
logn.perm.test_p_value[k] <- round(mean(abs(logn.permuted_statistics) >= abs(logn.observed_statistic)), 3)
gamma.perm.test_p_value[k] <- round(mean(abs(gamma.permuted_statistics) >= abs(gamma.observed_statistic)), 3)

#pwr_perm_norm[k] <- length( which( n.perm.test_p_value > alpha))/N
}
