...
perform_ds_func <- function(
    ...
    get_parameters = get_parameters,
    fn_to_get_norm_obj = fn_to_get_norm_obj,
    fn_for_norm_test   = normality_test,   # not used by ML ds_test_function
    fn_for_ds_test_1   = fn_for_ds_test_1,
    fn_for_ds_test_2   = fn_for_ds_test_2,
    norm_test_method = "SW",               # not used by ML ds_test_function
    ds_test_methods = ds_test_methods,
    effect_size = effect_size,
    indiv_threshold = indiv_threshold,                 # THIS acts as indiv_threshold now
    final_threshold = 0.60,                # add default here (or pass in)
    ...
) {
    ...
    for (method in ds_test_methods) {
      ...
      for (i in seq_along(sample_sizes)) {
        ...
        for (sim in seq_len(N)) {
          ...
          } else {# type I error for adaptive (ML classification) test
            p0 <- ds_test_function(
...
plot_power_by_effect_size <- function(power_results, 
  ...
  # Overall title
  mtext(sprintf("Power vs Effect Size | indiv_threshold = %.3f", indiv_threshold),
...
#                             RUN SIMULATIONS                                 #
# =============================================================================
run_simulation <- function(n, N, Nsim, test_type, distributions, tol_pos , indiv_threshold) {
  ...
  results <- list(
    params  = list(N = N, Nsim = Nsim, test_type = test_type, distributions = distributions , indiv_threshold = indiv_threshold),
    objects = list(),
    files   = list()
  )
  ...
  threshold_grid     <- seq(from = 0.009, to = 1, by = 0.0025)
