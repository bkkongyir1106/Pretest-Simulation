...
norm_statistic <- log_statistic <-t_statistic <- u_statistic<-e_statistic <- k_statistic <-g_statistic <- w_statistic <- NULL
 for(j in 1 : N){
   # normal distn
    ...
    norm_statistic[j] <- t.test(n1, n2 + d)$statistic
    ...
    e1 <- rexp(n, rate = 1)
    e2 <- rexp(n, rate = 1)
    e_statistic[j] <- t.test(e1 - 1, e2 - 1 + d)$statistic
    ...
    u2 <- runif(n, min = 0, max = 1)
      ...
      u_statistic[j] <- t.test((u1-0.5)/sqrt(1/12),  (u2-0.5)/sqrt(1/12) + d)$statistic
    ...
    t2 <-rt(n, df = 3) 
     ...
     t_statistic[j] <- t.test((t1)/sqrt(3), (t2)/sqrt(3) + d)$statistic
    ...
    k2 <-rchisq(n, df = 7) 
     ...
     k_statistic[j] <- t.test((k1 - 7)/sqrt(14), (k2 - 7)/sqrt(14) + d)$statistic
    ...
    g_statistic[j] <- t.test((g1-30)/sqrt(300), (g2-30)/sqrt(300) + d)$statistic
    ...
    w_statistic[j] <- t.test(u, w + d)$statistic
    ...
    log_statistic[j] <- t.test(z1, z2 + d)$statistic
...
hist(e_statistic, prob = T,  main = "Histogram of t test Statistics", sub = "Exponential Dist",
     xlab = "Value", ylab = "Density", ylim = c(0, 0.5), xlim = c(-5, 5))
