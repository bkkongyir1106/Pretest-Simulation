...
for(i in 1:num.sim){
  for (k in 1:length(deltavec)){
    # generate samples from each distn
...
```{r}
save.image(paste0("distn of t statistic under h1",".RData"))
```
...
### lognormal
```{r}
plot(density(tstat_chi[, 1]), typ = "n", xlim = c(-5, 10), ylim = c(0, 0.7), main = " ", xlab = " ")
