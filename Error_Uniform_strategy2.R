setwd("C:/Users/bkongyi/OneDrive - Oklahoma A and M System/Research")
set.seed(1)
alpha<-0.05
N<-100000
...
for(i in 1: length(sample_size)){
  ...
  for(j in 1: length(shapiro_alpha)){
    ...
    for(k in 1:N){
      ...
      {if(t.test(x1,x2)$p.value<alpha){reject_h0[k]<-1}}
      else if(shapiro.test(c(x1-mean(x1),x2-mean(x2)))$p.value<=alpha)
      {if(wilcox.test(x1,x2)$p.value<alpha){reject_h0[k]<-1}}
      else{ reject_h0[k]<-0}
      num_sim<-num_sim+1
    }
...
title(main = "Type I Error Rate -Exponential Distn")
...
Error_strategy2.exp<- as.data.frame(pvalue)

#export data into excel
library("writexl")
write_xlsx(Error_strategy2.exp,"C:/Users/bkongyi/OneDrive - Oklahoma A and M System/Research/Error.exp_II.xlsx")

save.image(paste0("exp.II_test",".RData"))
