...
myplot<-plot(sample_size,error[,1], type = "l", pch=24,lwd=3,ylim = c(0.00,0.10), 
             ylab = "Type I Error", xlab = "Sample Size")
title(main = "Type I Error Rate for two stage under strategy I Normal dist")
...
legend("topleft", legend=c("0.100", "0.050", '0.010',"0.005","w/o s-w test"),
       col=c("black","red","blue", "green"), lty =c(1:4),cex = 0.5)

error_norm_two_stage_I<- as.data.frame(error)

#export data into excel
library("writexl")
write_xlsx(error_norm_two_stage_I,"D:/OSU/Research_Fall2023/two stage strategy I/error_norm_two_stage_I.xlsx")

save.image(paste0("error_norm_two_stage_I",".RData"))
